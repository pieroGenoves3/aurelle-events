
export const de = {
  navigation: {
    home: "Startseite",
    mission: "Mission",
    contact: "Kontakt",
    about: "Über Mich",
    // events: "Veranstaltungen",
    // testimonials: "Referenzen",
    // about: "Über Uns", 
    // faq: "FAQ",
    // services: "Dienstleistungen"
  },
  hero: {
    title: "Aurelle Events",
    subtitle: "Außergewöhnliche Erlebnisse für Luxushochzeiten, Firmenevents und Haute-Couture-Modenschauen schaffen",
    cta: "Entdecken Sie Unsere Arbeit",
    missionQuote: "Unvergessliche Momente schaffen, die ein Leben lang halten",
    contactTitle: "Betreten Sie den Aurelle-Circle",
    contactSubtitle: "Aurelle richtet sich an jene, die das Wesentliche dem Überfluss vorziehen, die Reinheit der Form der blossen Inszenierung, die Präsenz der Darbietung.",
    contactInfo: "Kontaktinformationen",
    sendMessage: "Senden Sie uns eine Nachricht",
    nameField: "Name",
    emailField: "Email",
    countryCodeField: "Ländervorwahl",
    phoneNumberField: "Telefonnummer",
    subjectField: "Betreff",
    eventDateField: "Datum der Veranstaltung",
    preferedContactDateField: "Bevorzugter Zeitpunkt für die Kontaktaufnahme",
    messageField: "Nachricht",
    sendButton: "Private Beratung anfordern",
    sending: "Wird gesendet...",
    namePlaceholder: "Ihr Name",
    emailPlaceholder: "ihre@email.com",
    countryCodePlaceholder: "+49",
    phoneNumberPlaceholder: "30 901820",
    subjectPlaceholder: "Worum geht es?",
    eventDatePlaceholder: "Datum der Veranstaltung auswählen",
    preferedContactDatePlaceholder: "Beste Zeit für Kontaktaufnahme",
    messagePlaceholder: "Erzählen Sie uns mehr über Ihr Projekt oder Ihre Anfrage..."
  },
  mission: {
    title: "Unser Wesen",
    description: [
      "Bei Aurelle geben wir dem Unsichtbaren Form. Jede Feier wird als einzigartige Komposition konzipiert, präzise in der Struktur, tiefgreifend in der Präsenz, unmöglich zu wiederholen.","Wir gestalten keine Events. Wir choreographieren Atmosphären. Wir übersetzen Stille in Schönheit, Details in Bedeutung, Vision in Vermächtnis."
    ],
    quote: "Was wir schaffen, ist nicht erfunden. Es wird gefühlt. Es wirkt nach. Es wird zur Erinnerung, hallt in der Stille wider, besteht in der Zeit"
  },
  location: {
    title: "Finden Sie uns",
    subtitle: "Besuchen Sie unseren Standort",
    address: "Adresse",
    phone: "Telefon",
    email: "E-Mail",
    officeHours: "Bürozeiten",
    openInMaps: "In Karten öffnen"
  },
  events: {
    title: "Vergangene Events",
    subtitle: "Entdecken Sie unsere neuesten Arbeiten",
    guests: "Gäste",
    client: "Kunde",
    previousEvent: "Vorheriges Event",
    nextEvent: "Nächstes Event"
  },
  testimonials: {
    title: "Kundengeschichten",
    subtitle: "Was unsere Kunden über uns sagen",
    previousTestimonial: "Vorherige Bewertung",
    nextTestimonial: "Nächste Bewertung"
  },
  mottos: {
    title: "Unsere Werte",
    subtitle: "Was uns antreibt"
  },
  services: {
    underDevelopment: "In Entwicklung",
    underDevelopmentMessage: "Wir entwickeln derzeit diesen Service, um Ihnen außergewöhnliche Erlebnisse zu bieten. Bleiben Sie dran für spannende Updates!"
  },
  faq: {
    title: "Häufig gestellte Fragen",
    subtitle: "Finden Sie Antworten auf häufige Fragen zu unseren Dienstleistungen"
  },
  about: {
    title: "Über Mich",
    imageAlt: "Inhaberin von Aurelle Events",
    followMe: "Folgen Sie mir in den sozialen Medien",
    description: [
      "Willkommen bei Aurelle Events! Ich bin leidenschaftlich daran interessiert, außergewöhnliche Erlebnisse zu schaffen, die die kostbarsten Momente des Lebens feiern. Mit jahrelanger Erfahrung in der Luxus-Eventplanung glaube ich, dass jede Feier so einzigartig sein sollte wie die Menschen, die sie ehrt.",
      "Von intimen Zusammenkünften bis hin zu großen Feiern arbeite ich eng mit jedem Kunden zusammen, um seine Vision zum Leben zu erwecken. Meine Liebe zum Detail, mein kreativer Ansatz und mein Engagement für Exzellenz stellen sicher, dass jedes Event zu einer unvergesslichen Erinnerung wird.",
      "Lassen Sie uns gemeinsam etwas Schönes schaffen – denn jeder Moment verdient es, mit Eleganz und Raffinesse gefeiert zu werden."
    ]
  },
  footer: {
    leftTitle: "Bereit, etwas Außergewöhnliches zu erschaffen?",
    leftDescription: "Wo eine Geschichte mehr verlangt als ein Ereignis, entsteht eine Atmosphäre von seltener Schönheit und bleibender Bedeutung.",
    contactTitle: "Kontaktinformationen"
  },
  contact: {
    title: "Kontakt",
    subtitle: "Lassen Sie uns gemeinsam etwas Außergewöhnliches schaffen",
    name: "Name",
    email: "Email",
    message: "Nachricht",
    send: "Nachricht Senden",
    success: "Nachricht erfolgreich gesendet!",
    error: "Nachricht konnte nicht gesendet werden. Bitte versuchen Sie es erneut."
  }
};
