...
const MottosSection = () => {
  const { content, loading } = useContent('mottos');
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 scroll-reveal">
          <h2 className="text-4xl md:text-6xl font-light tracking-tighter mb-6">
            {content.title || "Our Philosophy"}
          </h2>
          <p className="text-xl text-foreground/80 max-w-2xl mx-auto">
            {content.subtitle || "The principles that guide every extraordinary event we create"}
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {content.mottos?.map((motto: any, index: number) => {
            const IconComponent = iconMap[motto.icon as keyof typeof iconMap] || Sparkles;
            return (
              <div
                key={index}
